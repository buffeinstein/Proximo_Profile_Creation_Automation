...
//   // Internal refs to store interval IDs so we can clear them
//   const jobIntervalRef = useRef<number | null>(null);
//   const snapshotIntervalRef = useRef<number | null>(null);

//   // Start handler: sends ingest request
//   const handleStart = useCallback(async () => {
//     setIngestError(null);
//     setIngestLoading(true);

//     try {
//       // Gather file if provided
//       const file = fileInputRef.current?.files?.[0] || null;

//       // Use FormData for file + simple fields
...
//       // (Optional) For the deterministic demo you could append a flag
//       // form.append("demo_mode", "true");

//       const resp = await fetch("/api/resumes/ingest", {
//         method: "POST",
//         body: form
//       });
...
//         throw new Error("Ingest response missing resumeId or jobId");
//       }

//       setResumeId(data.resumeId);
//       setJobId(data.jobId);

//       // Reset prior poll data if re-running
//       setJobProgress(null);
//       setSnapshot(null);
//     } catch (e: any) {
...
//     if (!resumeId) return;
//     try {
//       const r = await fetch(`/api/resumes/${resumeId}/snapshot`);
//       if (!r.ok) return;
//       const json = await r.json();
//       setSnapshot(json);
//     } catch {
...
//   // Effect: start polling when IDs first become available
//   useEffect(() => {
//     if (!resumeId || !jobId) return;

//     // Immediate first fetches (avoid waiting 1s)
//     pollJob();
//     pollSnapshot();
...
//     // Cleanup on IDs change or unmount
//     return () => {
...
//   // Effect: stop polling when job completes
//   useEffect(() => {
...
//       // Optional: one final snapshot fetch to be sure everything is fully enriched
//       pollSnapshot();
//     }
...
//       ? 0
//       : Math.round(
//           (jobProgress.completed_roles / jobProgress.total_roles) * 100
//         )
//     : 0;

//   return (
//     <div style={{ display: "flex", height: "100vh", fontFamily: "Arial, sans-serif" }}>
//       {/* Left: Snapshot / Profile */}
//       <div style={{ flex: 1, padding: 20, borderRight: "1px solid #ccc", overflowY: "auto" }}>
//         <h1>User Profile Snapshot</h1>
//         {!snapshot && <p style={{ opacity: 0.7 }}>No snapshot yet. Start a run to see data.</p>}

//         {snapshot && (
//           <div>
//             <h2>{snapshot.full_name || "(Name pending)"}</h2>
//             <h3>Roles</h3>
//             {snapshot.roles.length === 0 && <p>No roles parsed.</p>}
...
//                 >
//                   <strong>
//                     {role.title || "(title pending)"} @ {role.company || "(company pending)"}
//                   </strong>
//                   <div style={{ fontSize: 12, color: "#555", marginTop: 4 }}>
//                     Industry: {role.role_industry || "—"} | Seniority: {role.role_seniority || "—"}
//                   </div>
//                   <div style={{ fontSize: 12, color: "#555" }}>
//                     Company Size: {role.company_size || "—"} | Company Industry: {role.company_industry || "—"}
//                   </div>
//                   <div style={{ marginTop: 8 }}>
//                     <em>Enriched field count: {enrichedFields}</em>
//                   </div>
//                   <ul style={{ marginTop: 8 }}>
//                     {role.role_star_1 && <li>{role.role_star_1}</li>}
//                     {role.role_star_2 && <li>{role.role_star_2}</li>}
//                     {role.role_star_3 && <li>{role.role_star_3}</li>}
//                     {role.metric_1 && <li>Metric 1: {role.metric_1}</li>}
//                     {role.metric_2 && <li>Metric 2: {role.metric_2}</li>}
//                     {role.metric_3 && <li>Metric 3: {role.metric_3}</li>}
//                   </ul>
//                 </div>
//               );
//             })}
...
//             </div>
//             <div style={{ height: 14, background: "#eee", borderRadius: 8, overflow: "hidden", marginBottom: 4 }}>
//               <div
//                 style={{
